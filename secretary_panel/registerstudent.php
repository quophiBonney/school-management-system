...
if(isset($_POST['register'])) {
    ...
    if($studentName == "$studentName" && $contact == "$contact") {
        $isStudentnameTaken = "SELECT * FROM student_registration WHERE studentName = '$studentName' AND contact = '$contact'";
        $result = mysqli_query($conn, $isStudentnameTaken);
        $num = mysqli_num_rows($result);
        if($num > 0) {
            echo '<script>alert("Student info has already been taken")</script>';
        }else {
            $sql = "INSERT INTO student_registration(studentName, course, email, contact, form, dob, parentName, parentMobile, image)
            VALUES('$studentName', '$course', '$email', '$contact', '$form', '$dob', '$parentName', '$parentMobile', '$filename')";
...
<div class="container justify-content-center">
    <div class="row justify-content-center">
        <div class="col-md-6 p-3 my-4 shadow" id="login-section">
            <h4 class="text-center p-3">STUDENT REGISTRATION FORM</h4>
            <hr>
        <form action="registerstudent.php" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <input type="text" name="course" placeholder="Course/Programme" required="" class="form-control" autocomplete="off">
            </div>
            <div class="form-group">
                <input type="email" name="email" placeholder="Email" required="" class="form-control" autocomplete="off">
            </div>
            <div class="form-group">
                <input type="text" name="contact" placeholder="Contact" required="" class="form-control" autocomplete="off">
            </div>
            <div class="form-group">
                <input type="text" name="form" placeholder="Form" required="" class="form-control" autocomplete="off">
            </div>
            <div class="form-group">
