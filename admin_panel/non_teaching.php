...
if(isset($_POST['register'])) {
    ...
    if($email == "$email" && $image == "image") {
        $isStaffnameTaken = "SELECT * FROM non_teaching WHERE email = '$email' AND image = '$image'";
        $result = mysqli_query($conn, $isStaffnameTaken);
        $num = mysqli_num_rows($result);
        if($num > 0) {
            echo '<script>alert("Staff has already been registered")</script>';
        }else {
            $sql = "INSERT INTO non-teaching(staffName, email, contact, department, image)
            VALUES('$staffName', '$email', '$contact', '$department', '$image')";
...
<div class="container-fluid justify-content-center my-4 p-3">
    <div class="row justify-content-center">
        <div class="col-md-6 p-3 my-3 shadow" id="login-section">
