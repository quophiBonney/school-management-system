...
if(isset($_POST['present'])) {
    ...
    if($studentName == "$studentName") {
        $isStudentnameTaken = "SELECT * FROM dailyRegister WHERE studentName = '$studentName'";
        $result = mysqli_query($conn, $isStudentnameTaken);
        $num = mysqli_num_rows($result);
      if($num > 0) {
        echo '<script>alert("Student have already reported")</script>';
    }else {
        $sql = "INSERT INTO dailyRegister(studentName)
         VALUES('$studentName')";
...
<div class="container justify-content-center my-4 p-3">
    <div class="row justify-content-center">
        <div class="col-md-6 p-3 my-4 shadow" id="login-section">
            <h4 class="text-center p-3">DAILY REGISTER</h4>
            <hr>
        <form action="student_form.php" method="POST">
            ...
            <div class="form-group">
                <input type="submit" name="present" value="PRESENT" class="form-control" style="background-color: rgb(23, 91, 180); color: white";>
            </div>
        </form>
</div>
</body>
